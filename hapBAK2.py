...
def buildFromDataDir(path = home_dir+"/data"):
    work = {}
    os.chdir(path)
    dir = os.listdir()
    for i in dir:
        if i[-6:]==".happy" and os.path.isfile(i):
            pass
        if os.path.isdir(i):
            pass
    return work
...
outer_fail = "Happy does not know what that first word is; run happy help to see what can be said."
welcome = "*happy says 'hello' and is ready for your requests* (maybe try 'happy help'?)"
outer_subs = buildFromDataDir()

outer_OU = OU(outer_fail, welcome, outer_subs)


BAKouter_OU = OU("Happy does not know what that first word is; run happy help to see what can be said.", welcome, {
